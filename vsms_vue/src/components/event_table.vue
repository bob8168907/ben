    ...
    export default {
        ...
        computed:{
            notapad(){
                var self = this;
               var notapad = [];
               axios.get('http://localhost:8888/vsmsserver/customer/customer/list?uuid=2340&page=1&start=0&limit=25').then((res) => {
                    var datas = res.data.recordList; 
                    for(var i in datas){
                        var param = {};
                        param.customername = datas[i].customername;
                        param.checkername = datas[i].checkername;
                        param.province = datas[i].province;
                        notapad.push(param);
                    }
